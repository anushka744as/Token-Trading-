...
const Header: React.FC = () => {
  ...
  // Define static navigation links (Next.js way)
  const navigation = [
    { name: "Home", path: "/" },
    { name: "Sample Page", path: "/SamplePage" },
          ...
          <div className="hidden md:flex items-center space-x-8">
            {navigation.map((item) => (
              <Link
                key={item.path}
                href={item.path}
                className="px-3 py-2 text-base font-medium text-gray-700 hover:text-blue-600 hover:bg-gray-50 rounded-md transition duration-300"
              >
                {item.name}
              </Link>
            ))}
